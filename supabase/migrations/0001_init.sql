-- Single setup script for a fresh Supabase project.
-- Run once in SQL Editor. All tables, RLS, trigger, and seed data included.
-- Admin account: sign up at /auth/register with admin@naver.com → auto role='admin'
...
-- Policies on profiles cannot SELECT from profiles themselves (infinite loop).
...
create policy "profiles update admin" on public.profiles for update using (
...
create policy "works select all"   on public.works for select using (true);
create policy "works insert admin" on public.works for insert with check (
  exists (select 1 from public.profiles where id = auth.uid() and role = 'admin')
);
create policy "works update admin" on public.works for update using (
  exists (select 1 from public.profiles where id = auth.uid() and role = 'admin')
);
create policy "works delete admin" on public.works for delete using (
  exists (select 1 from public.profiles where id = auth.uid() and role = 'admin')
);

-- ── storage RLS: works-files bucket ──────────────────────────────────────────
-- Public bucket → unauthenticated GET via /object/public/... is allowed by default.
-- Only admins may upload or delete via the Storage API.
...
-- ── seed data ─────────────────────────────────────────────────────────────────
-- Posts / comments: user_id = NULL (demo content, publicly readable, no owner)
...
-- Works: readable by all, editable by admin@naver.com after login
