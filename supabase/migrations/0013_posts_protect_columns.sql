...
--       (반대로 앱의 조회수/좋아요 증가는 클라이언트가 직접 UPDATE하는 방식이라, RLS 때문에
--        작성자·관리자가 아닌 사람의 조회/좋아요는 DB에 저장되지 않고 있었음.)
--
-- 해결:
--  1) BEFORE INSERT/UPDATE 트리거로, 클라이언트(anon/authenticated 롤)가 관리자가 아니면
--     is_hidden/view_count/like_count를 바꾸지 못하게 막음. 서비스 키·SQL Editor·아래 RPC는 통과.
--  2) 조회수/좋아요는 security definer RPC로만 +1 하게 해서 누구의 조회/좋아요든 저장되게 함.
-- ============================================================
...
-- 좋아요 +1: 공개된 글만, 로그인한 사용자만
create or replace function public.increment_post_like(_post_id uuid)
returns void language sql security definer set search_path = public as $$
