...
- [배포 전 필수] D-DAY 기능(로컬 커밋, 미 push): 운영 DB에 `supabase/migrations/0017_schedule_dday.sql`(`schedules.is_dday` 컬럼 추가)을 **먼저 적용한 뒤** push할 것. 코드가 `is_dday`를 select하므로 적용 전에 배포하면 일정 페이지가 빈 화면이 됨. 적용 뒤 확인: 일정 추가/수정에서 "D-DAY로 표시" 저장, 일정 페이지 `D-n` 알약, 홈 최근 활동의 D-DAY 카드(가까운 3개), 반복 일정에서 체크박스 비활성
