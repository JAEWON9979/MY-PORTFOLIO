...
## 2026-09-20
- 홈 소개 섹션(`About.tsx`) 레이아웃 변경: 제목 + 한 문장뿐이라 홈에서 가장 밋밋했음. 세 방향(결합 강조형/좌우 2단 프로필형/큰 타이포형) 중 사용자가 "결합 강조형"을 골라 진행. 첫 문장은 이름만 굵게 키운 리드 문장으로, 그 아래에 "행정 역량(기본) + AI 역량(결합 목표)" 두 카드를 "+"로 잇고(AI 카드는 진한 테두리·배경으로 강조), 기존 문장("행정역량을 기본으로 AI역량을 결합하는것을 목표하고 있습니다.")은 작은 보조 문구로 유지. 데스크톱은 가로 2열, `sm` 미만은 세로로 쌓임. 홈의 기존 톤(zinc 팔레트, 둥근 칩, framer-motion 페이드인, `max-w-3xl`)에 맞춤. 새 사실을 지어내지 않으려고 카드 문구는 기존 정보(Skills의 행정 업무 항목, 사용자의 목표 문장)에서만 가져옴 — 카드 설명 두 줄은 사용자가 다듬을 수 있음. 카드 설명은 `break-keep`으로 한글이 단어 중간에서 줄바꿈되지 않게 함. 헤드리스 Chrome으로 데스크톱 캡처 확인, 좁은 폭(560px)은 세로 쌓임 구조까지만 확인(캡처 시점에 애니메이션이 진행 중이라 카드 최종 모습은 못 봄, 실제 모바일 화면은 사용자가 확인 필요). lint/build 통과
